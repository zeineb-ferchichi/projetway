    ...
    public class AfficherForum {
        ...
        @FXML
        void initialize() {
            System.out.println("AfficherForum initialized. isAdmin = " + isAdmin);
        ...
        private void refreshForums() {
           ...
           System.out.println(isAdmin);
            ...
            if (!Session.isAdmin()) {
                btnAjouterInterface.setVisible(true);
                btnAjouterInterface.setManaged(true);
            }
            try {
                ...
                for (Forum forum : forums) {
                    ...
                    int commentCount = forumService.countByForumId(forum.getIdForum());
                    Label commentLabel = new Label("Commentaires: " + commentCount);
                    commentLabel.setStyle("-fx-text-fill: #555555; -fx-font-size: 12px;");
                    forumBox.getChildren().add(commentLabel);

                    Label likesLabel = new Label("Likes: " + forum.getLikes());
                    likesLabel.setStyle("-fx-text-fill: #555555; -fx-font-size: 12px;");

                    Button btnLike = new Button("Like");
                    btnLike.setStyle("-fx-background-color: #28a745; -fx-text-fill: white;");
                    btnLike.setOnAction(e -> {
                        forum.increaseLikes(); // Update local object
                        forumService.update(forum); // Save to DB
                        likesLabel.setText("Likes: " + forum.getLikes()); // Refresh label
                    });
                    Button btnDisLike = new Button("Dislike");
                    btnDisLike.setStyle("-fx-background-color: #FF0000; -fx-text-fill: white;");
                    btnDisLike.setOnAction(e -> {
                        forum.decreaseLikes(); // Update local object
                        forumService.update(forum); // Save to DB
                        likesLabel.setText("Likes: " + forum.getLikes()); // Refresh label
                    });
                    HBox dislikeContainer = new HBox(20, likesLabel);
                    HBox likeContainer = new HBox(10, btnDisLike, btnLike);
                    forumBox.getChildren().add(likeContainer);
                    forumBox.getChildren().add(dislikeContainer);
                    ...
                    if (Session.isAdmin()) {
                        forumBox.getChildren().add(btnAjouterInterface); // only admins can add

                        HBox buttonContainer = new HBox(10, btnModifier, btnSupprimer, btnMessage);
                        forumBox.getChildren().add(buttonContainer);
                    } else {
                        forumBox.getChildren().add(btnMessage); // basic user can only message and like
                    }
        ...
        public void setIsAdmin(boolean isAdmin) {
            ...
            refreshForums();
            System.out.println("Admin flag set to: " + isAdmin);
        }
    }
